            ...
            <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4'>
              {skill.items.map((item) => (
                <motion.div
                  ...
                  viewport={{ once: true }}
                  className='flex items-center gap-3 p-4 bg-gradient-to-r from-black/30 to-stone-900/50 backdrop-blur-lg rounded-xl shadow-stone-600 shadow-sm hover:shadow-md transition duration-300 ease-in-out'
                >
                  <div className='text-3xl'>{item.icon}</div>
                  <span className='text-stone-300 font-medium text-base'>
                    {item.name}
                  </span>
                </motion.div>
              ))}
